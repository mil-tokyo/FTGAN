...
def gan_training(args, train, test):
    ...
    np.random.seed(0)
    train_indices = np.random.randint(0, len(train), args.rows*args.cols).tolist()
    ...
    trainer.extend(visualizer.extension(train, gen, train_indices, args, 'train', rows=args.rows, cols=args.cols),
                   trigger=visualize_interval)
    trainer.extend(visualizer.extension(test, gen, test_indices, args, 'test', rows=args.rows, cols=args.cols),
                   trigger=visualize_interval)
...
def main():
    ...
    parser.add_argument('--out', '-o', default='../result_texgan/',
    ...
    parser.add_argument('--weight_decay', '-wd', type=int, default=0,
                        help='')
    parser.add_argument('--alpha', '-a', type=float, default=0.0002,
                        help='')
