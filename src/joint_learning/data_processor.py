...
class PreprocessedDataset(chainer.dataset.DatasetMixin):
    def __init__(self, dataset, video_path, flow_path, video_len=32, crop_size=(64,64), flow_len=11):
        self.dataset = dataset
        self.flow_dir = flow_path
        self.video_path = video_path
        self.video_len = video_len
        self.crop_y = crop_size[0]
        self.crop_x = crop_size[1]
        self.flow_len = flow_len
        self.stride=video_len//2
    ...
    def preprocess_img(self, image, flow, flipcrop=True):
        ...
        if flipcrop:
            ...
            for i in range(image.shape[1]):
                dst_image[:, i] = scipy.misc.imresize(image[:, i, :, :].transpose(1, 2, 0), [self.crop_y, self.crop_x],
